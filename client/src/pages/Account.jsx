...
const Account = () => {
  ...
  const toggleDrawer = (type = null) => {
    let classes =
      "flex justify-center items-center gap-2 py-2 px-6 text-[16px] font-medium cursor-pointer text-gray-700";
    ...
    <div className="w-full h-screen justify-center mx-auto sm:min-w-0 sm:flex">
      <div className="flex flex-col w-full">
        ...
        <nav className="flex flex-wrap justify-center mt-8 gap-4">
          <Link to="/account" className={`${toggleDrawer("profile")}`}>
            <AiOutlineUser /> Profile
          </Link>
          <Link to="/account/places" className={`${toggleDrawer("places")}`}>
            <BsBuildingAdd /> Accommodations
          </Link>
          <Link
            to="/account/bookings"
            className={`${toggleDrawer("bookings")}`}
          >
            <BsCalendar2Plus /> Booking
          </Link>
        </nav>

        {subpage === "profile" && (
          <div className="flex justify-center items-center mx-auto mt-8">
            <p className="text-center text-gray-700 text-[18px] font-medium">
